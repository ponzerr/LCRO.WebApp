      ...
      <div class="overflow-x-auto">
        <table class="w-full table-auto text-left text-sm font-light bg-white border border-gray-300">
          <thead class="border-b bg-white font-medium">
            <tr>
              <th scope="col" class="px-6 py-3 text-center">Registry No.</th>
          ...
          <tbody>
            ...
            @if($mcerts->count() || $mcerts->isNotEmpty())
              @foreach ($mcerts as $mcert)
                <tr class="border-b bg-neutral-100">
                  <th class="whitespace-nowrap px-6 py-2 text-center">{{ $mcert->mcert_registry_no }}</th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $mcert->mcert_g_first_name }} {{ $mcert->mcert_g_middle_name }} {{ $mcert->mcert_g_last_name }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $mcert->mcert_b_first_name }} {{ $mcert->mcert_b_middle_name }} {{ $mcert->mcert_b_last_name }}</p></th>
                  <!-- Add more table cells for other fields -->
                  <th class="whitespace-nowrap px-6 py-2 text-center">
                    <!-- show button -->
                    <a href="{{ route('mcerts.show', $mcert) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- edit button -->
                    <a href="{{ route('mcerts.edit', $mcert) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- delete button -->
                    <form action="{{ route('mcerts.destroy', $mcert) }}" class="inline-block mb-1 ml-4" method="POST" onsubmit="return confirm('Are you sure you want to delete this Mcert?')">
                      @csrf
                      @method('DELETE')
