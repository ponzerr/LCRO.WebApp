<!-- resources/views/bcerts/index.blade.php -->
...
<div class="flex flex-col justify-left items-left mx-1 mt-4">
    <p class="text-gray-500 text-m text-center">Republic of the Philippines</p>
        ...
        <h2 class="font-semibold text-xl mb-2 text-gray-600 text-center">CERTIFICATE OF BIRTH</h2>
      <div class="flex items-center justify-center mx-8">
          
          <!-- upload file button -->
          <a href="{{ route('bcerts.pdf') }}" class="ml-2 inline-block">
      ...
      <div class="flex items-center justify-center mx-8 mt-2">
          <div class="mr-1">
          <form action="{{ route('generate-birth-report') }}" method="POST">
            @csrf
            <div class="w-23">
                <input
                type="date"
    ...
    <div
        class="mt-4 font-regular relative w-full rounded-lg bg-red-500 px-4 py-4 text-base text-white"
        data-dismissible="alert"
      >
        <div class="absolute top-4 left-4">
          <svg
      ...
      <div class="overflow-x-auto">
        <table class="w-full table-auto text-left text-sm font-light bg-white border border-gray-300">
          <thead class="border-b bg-white font-medium">
            <tr>
              <th scope="col" class="px-6 py-3 text-center">Birth Registry No.</th>
              <th scope="col" class="px-6 py-3 text-center">Birth Name</th>
          ...
          <tbody>
            @if($bcertFiles->count()) 
              @foreach ($bcertFiles as $bcertFile)
                <tr class="border-b bg-neutral-100">
                  <th class="whitespace-nowrap px-6 py-2 text-center">{{ $bcertFile->bcert_file_registry_no }}</th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $bcertFile->bcert_file_birth_name }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $bcertFile->bcert_file_attach_document }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $bcertFile->bcert_file_date_of_birth }}</p></th>
                  <!-- Add more table cells for other fields -->
                  <th class="whitespace-nowrap px-6 py-2 text-center">
                    <!-- show button -->
                    <a href="{{ route('bcerts.show_file', $bcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- edit button -->
                    <a href="{{ route('bcerts.edit_file', $bcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- delete button -->
                    <form action="{{ route('bcerts.destroy_file', $bcertFile) }}" class="inline-block mb-1 ml-4" method="POST" onsubmit="return confirm('Are you sure you want to delete this Bcert?')">
                      @csrf
                      @method('DELETE')
           ...
           @else
              <tr>
                <td colspan="6" class="py-4 text-center">No records found.</td>
              </tr>
            @endif
          </tbody>
        </table>
