<!-- resources/views/dcerts/index.blade.php -->
...
<div class="flex flex-col justify-left items-left mx-1 mt-4">
    <p class="text-gray-500 text-m text-center">Republic of the Philippines</p>
        ...
        <h2 class="font-semibold text-xl mb-2 text-gray-600 text-center">CERTIFICATE OF DEATH</h2>
      <div class="flex items-center justify-center mx-8">
          
          <!-- upload file button -->
          <a href="{{ route('dcerts.pdf') }}" class="ml-2 inline-block">
      ...
      <div class="flex items-center justify-center mx-8 mt-2">
          <div class="mr-1">
          <form action="{{ route('generate-death-report') }}" method="POST">
            @csrf
            <div class="w-23">
                <input
                type="date"
      ...
      <div class="overflow-x-auto">
        <table class="w-full table-auto text-left text-sm font-light bg-white border border-gray-300">
          <thead class="border-b bg-white font-medium">
            <tr>
              <th scope="col" class="px-6 py-3 text-center">Death Registry No.</th>
          ...
          <tbody>
            @if($dcertFiles->count()) 
              @foreach ($dcertFiles as $dcertFile)
                <tr class="border-b bg-neutral-100">
                  <th class="whitespace-nowrap px-6 py-2 text-center">{{ $dcertFile->dcert_file_registry_no }}</th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $dcertFile->dcert_file_death_name }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $dcertFile->dcert_file_attach_document }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $dcertFile->dcert_file_date_of_death }}</p></th>
                  <!-- Add more table cells for other fields -->
                  <th class="whitespace-nowrap px-6 py-2 text-center">
                    <!-- show button -->
                    <a href="{{ route('dcerts.show_file', $dcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- edit button -->
                    <a href="{{ route('dcerts.edit_file', $dcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- delete button -->
                    <form action="{{ route('dcerts.destroy_file', $dcertFile) }}" class="inline-block mb-1 ml-4" method="POST" onsubmit="return confirm('Are you sure you want to delete this Dcert?')">
                      @csrf
                      @method('DELETE')
