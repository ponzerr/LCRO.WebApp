<!-- resources/views/bcerts/edit.blade.php -->
...
<div class="min-h-screen p-6 mt-1 bg-gray-100 flex items-center justify-center">
      <div class="container max-w-screen-lg mx-auto">
        ...
        <form class="mt-8 space-y-3" action="{{ route('bcerts.update_file', $bcertFile) }}" method="POST">
            @method('PUT')
            @csrf
				...
				<div class="lg:col-span-6">
              		<div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
						<div class="md:col-span-6">
							...
							<input type="text" name="bcert_file_registry_no" id="bcert_file_registry_no" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" 
              value="{{ $bcertFile->bcert_file_registry_no }}" placeholder=""/>
						</div>

						<div class="md:col-span-6">
							<label for="bcert_file_birth_name" class="text-sm font-bold text-gray-500 tracking-wide">Birth Name</label>
							<input type="text" name="bcert_file_birth_name" id="bcert_file_birth_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" 
              value="{{ $bcertFile->bcert_file_birth_name }}" placeholder=""/>
						</div>
						
						<div class="md:col-span-6">
							<label for="bcert_file_date_of_birth" class="text-sm font-bold text-gray-500 tracking-wide">Date of Birth</label>
							<input type="date" name="bcert_file_date_of_birth" id="bcert_file_date_of_birth" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" 
              value="{{ $bcertFile->bcert_file_date_of_birth }}" placeholder=""/>
						</div>
					</div>
				</div>
			</div>
			<div class="grid grid-cols-1 space-y-2">
				<label class="text-sm font-bold text-gray-500 tracking-wide">Attached Document</label>
					<div class="md:col-span-6">
                            ...
                            <a href="{{ asset('bcerts/' . $bcertFile->bcert_file_path) }}" target="_blank" class="text-blue-600 hover:underline">
