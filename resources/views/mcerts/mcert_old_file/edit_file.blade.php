...
<div class="min-h-screen p-6 mt-1 bg-gray-100 flex items-center justify-center">
      <div class="container max-w-screen-lg mx-auto">
        ...
        <form class="mt-8 space-y-3" action="{{ route('mcerts.update_file', $mcertFile) }}" method="POST">
            @method('PUT')
            @csrf
				...
				<div class="lg:col-span-6">
              		<div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
						<div class="md:col-span-6">
							...
							<input type="text" name="mcert_file_registry_no" id="mcert_file_registry_no" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" 
              value="{{ $mcertFile->mcert_file_registry_no }}" placeholder=""/>
						</div>

						<div class="md:col-span-6">
							<label for="mcert_file_groom_name" class="text-sm font-bold text-gray-500 tracking-wide">Groom Name</label>
							<input type="text" name="mcert_file_groom_name" id="mcert_file_groom_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" 
              value="{{ $mcertFile->mcert_file_groom_name }}" placeholder=""/>
						</div>
