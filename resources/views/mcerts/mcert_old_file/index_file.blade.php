...
<div class="flex flex-col justify-left items-left mx-1 mt-4">
    <p class="text-gray-500 text-m text-center">Republic of the Philippines</p>
        ...
        <h2 class="font-semibold text-xl mb-2 text-gray-600 text-center">CERTIFICATE OF MARRIAGE</h2>
      <div class="flex items-center justify-center mx-8">
          
          <!-- upload file button -->
          <a href="{{ route('mcerts.pdf') }}" class="ml-2 inline-block">
      ...
      <div class="flex items-center justify-center mx-8 mt-2">
          <div class="mr-1">
          <form action="{{ route('generate-legacy-report') }}" method="POST">
            @csrf
            <div class="w-23">
                <input
                type="date"
    ...
    <!-- <div class="inline-block min-w-full py-2 sm:px-6"> -->
      <div class="overflow-x-auto">
        <table class="w-full table-auto text-left text-sm font-light bg-white border border-gray-300">
          ...
          <tbody>
            @if($mcertFiles->count()) 
              @foreach ($mcertFiles as $mcertFile)
                <tr class="border-b bg-neutral-100">
                  <th class="whitespace-nowrap px-6 py-2 text-center">{{ $mcertFile->mcert_file_registry_no }}</th>
                  <th class="whitespace-nowrap px-6 py-2 text-center">{{ $mcertFile->mcert_file_groom_name }}</th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $mcertFile->mcert_file_bride_name }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $mcertFile->mcert_file_attach_document }}</p></th>
                  <th class="whitespace-nowrap px-6 py-2 text-center"><p>{{ $mcertFile->mcert_file_date_of_marriage }}</p></th>
                  <!-- Add more table cells for other fields -->
                  <th class="whitespace-nowrap px-6 py-2 text-center">
                    <!-- show button -->
                    <a href="{{ route('mcerts.show_file', $mcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- edit button -->
                    <a href="{{ route('mcerts.edit_file', $mcertFile) }}" class="inline-block mb-1 ml-4">
                    ...
                    <br>
                    <!-- delete button -->
                    <form action="{{ route('mcerts.destroy_file', $mcertFile) }}" class="inline-block mb-1 ml-4" method="POST" onsubmit="return confirm('Are you sure you want to delete this Mcert?')">
                      @csrf
                      @method('DELETE')
