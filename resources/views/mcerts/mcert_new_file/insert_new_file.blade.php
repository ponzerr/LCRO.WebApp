...
<div class="min-h-screen p-6 mt-1 bg-gray-100 flex items-center justify-center">
    ...
    <form class="mt-8 space-y-3" action="{{ route('mcerts.pdf.file_new') }}" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="grid grid-cols-1 space-y-6">
        <div class="lg:col-span-6">
          <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
            <div class="md:col-span-6">
              <label for="mcert_new_file_registry_no" class="text-sm font-bold text-gray-500 tracking-wide">Marriage Registery No.</label>
              <input type="text" name="mcert_new_file_registry_no" id="mcert_new_file_registry_no" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="" placeholder="" required/>
            </div>
            <div class="md:col-span-6">
              <label for="mcert_new_file_groom_name" class="text-sm font-bold text-gray-500 tracking-wide">Groom Name</label>
              <input type="text" name="mcert_new_file_groom_name" id="mcert_new_file_groom_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="" placeholder="" required/>
            </div>
            <div class="md:col-span-6">
              <label for="mcert_new_file_bride_name" class="text-sm font-bold text-gray-500 tracking-wide">Bride Name</label>
              <input type="text" name="mcert_new_file_bride_name" id="mcert_new_file_bride_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="" placeholder="" required/>
            </div>
          </div>
        </div>
      </div>
      <div class="grid grid-cols-1 space-y-2">
        ...
        <div class="border-2 border-gray-300 border-dashed rounded-md p-4">
          ...
          <div class="mt-4 mb-2 flex justify-center items-center">
            <input name="mcert_new_file_attach_document" type="file" class="" id="file" />
          </div>
        </div>
